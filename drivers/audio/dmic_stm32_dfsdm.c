...
struct dmic_dfsdm_filter_data {
	...
	struct dmic_dfsdm_filter_osr osr[2];	/* 0: regular, 1: fast conversion mode */
...
struct dmic_dfsdm_filter_cfg {
	void (*irq_config_func)(const struct device *dev);
	const struct pinctrl_dev_config *pcfg;
	const struct device *parent;
};

struct dmic_dfsdm_cfg {
...
static void dmic_dfsdm_isr(const struct device *dev)
{
	struct dmic_dfsdm_filter_data *data = dev->data;
...
{
	struct dmic_dfsdm_filter_data *data =
		CONTAINER_OF(hdfsdm_filter, struct dmic_dfsdm_filter_data, hfilter);
 ...
 */
static int dmic_dfsdm_compute_osrs(struct dmic_dfsdm_filter_osr *osr, uint32_t ford,
	...
	for (fosr = 1; fosr <= DFSDM_MAX_FL_OVERSAMPLING; fosr++) {
		for (iosr = 1; iosr <= DFSDM_MAX_INT_OVERSAMPLING; iosr++) {
			...
			if (res >= osr->res) {
				...
				/* 8 LSB in data register contain chan info */
				max = osr->res << 8;
...
static int dmic_dfsdm_get_all_osrs(const struct device *dev,
				   unsigned int oversamp)
{
	...
	ret = dmic_dfsdm_compute_osrs(&data->osr[0], data->sincorder, 0, oversamp);
	if (ret < 0) {
	...
	ret = dmic_dfsdm_compute_osrs(&data->osr[1], data->sincorder, 1, oversamp);
	if (ret < 0) {
...
static int dmic_dfsdm_stop(const struct device *dev)
{
	struct dmic_dfsdm_filter_data *data = dev->data;
...
static int dmic_dfsdm_start(const struct device *dev)
{
	struct dmic_dfsdm_filter_data *data = dev->data;
	HAL_StatusTypeDef hal_ret;
	int ret;
...
static int dmic_dfsdm_filter_deinit(const struct device *dev)
{
	struct dmic_dfsdm_filter_data *data = dev->data;
	int ret = 0;

	if (data->state == DMIC_STATE_ACTIVE) {
		ret = dmic_dfsdm_stop(dev);
	}
...
static int dmic_dfsdm_trigger(const struct device *dev, enum dmic_trigger cmd)
{
	struct dmic_dfsdm_filter_data *data = dev->data;
	int ret;
	...
	case DMIC_TRIGGER_RESET:
		ret = dmic_dfsdm_filter_deinit(dev);
		if (ret < 0) {
...
static int dmic_dfsdm_read(const struct device *dev, uint8_t stream, void **buffer,
...
static int dmic_dfsdm_setup_channel(const struct device *dev, uint32_t div,
...
static int dmic_dfsdm_setup_filter(const struct device *dev, uint8_t chan)
{
	struct dmic_dfsdm_filter_data *data = dev->data;
	DFSDM_Filter_HandleTypeDef *hfilter = &data->hfilter;
	HAL_StatusTypeDef hal_ret;
...
static int dmic_dfsdm_filter_init(const struct device *dev)
{
	const struct dmic_dfsdm_filter_cfg *cfg = dev->config;
	struct dmic_dfsdm_filter_data *data = dev->data;
	int ret;
...
static int dmic_dfsdm_configure(const struct device *dev, struct dmic_cfg *cfg)
{
	const struct dmic_dfsdm_filter_cfg *drv_cfg = dev->config;
	const struct dmic_dfsdm_cfg *parent_cfg = drv_cfg->parent->config;
	struct dmic_dfsdm_filter_data *data = dev->data;
	...
	if (data->state == DMIC_STATE_UNINIT) {
		ret = dmic_dfsdm_filter_init(dev);
		if (ret < 0) {
	...
	if (!IN_RANGE(bitclk_rate,
		      cfg->io.min_pdm_clk_freq + 1,
		      cfg->io.max_pdm_clk_freq - 1)) {
		/* Clamp prescaler values to hardware limits */
		min = MAX(1, DIV_ROUND_UP(bitclk_rate, cfg->io.max_pdm_clk_freq));
		max = MIN(255, bitclk_rate / cfg->io.min_pdm_clk_freq);
	...
	ret = dmic_dfsdm_get_all_osrs(dev, oversamp);
	if (ret < 0) {
		return ret;
	}
	...
	ret = dmic_dfsdm_setup_channel(dev, min, hw_chan, lr);
	if (ret < 0) {
		return ret;
	}
	...
	ret = dmic_dfsdm_setup_filter(dev, hw_chan);
	if (ret < 0) {
		return ret;
	}
...
static DEVICE_API(dmic, dmic_dfsdm_ops) = {
	.configure = dmic_dfsdm_configure,
	.trigger = dmic_dfsdm_trigger,
	.read = dmic_dfsdm_read,
};

static int dmic_dfsdm_init(const struct device *dev)
{
	const struct dmic_dfsdm_cfg *cfg = dev->config;
	int ret;
...
static int dmic_dfsdm_deinit(const struct device *dev)
{
	const struct dmic_dfsdm_cfg *cfg = dev->config;
	int ret = 0;
...
static int dmic_dfsdm_pm_action(const struct device *dev,
				enum pm_device_action action)
{
	switch (action) {
	case PM_DEVICE_ACTION_RESUME:
		return dmic_dfsdm_init(dev);
	case PM_DEVICE_ACTION_SUSPEND:
		return dmic_dfsdm_deinit(dev);
	default:
		return -ENOTSUP;
	}
}
...
#define DMIC_DFSDM_FILTERS_DEFINE(flt)                                         \
	PINCTRL_DT_DEFINE(flt);                                                \
                                                                               \
	K_MSGQ_DEFINE(dmic_dfsdm_msgq_##flt, sizeof(void *),                   \
	...
	static DFSDM_Channel_HandleTypeDef hchannels_##flt[] = {               \
		DT_FOREACH_CHILD(flt, DMIC_DFSDM_CHAN_DEFINE)                  \
                                                                               ...
                                                                               \
	static void dmic_dfsdm_irq_cfg_func_##flt(const struct device *dev)    \
	{                                                                      \
		IRQ_CONNECT(DT_IRQN(flt), DT_IRQ(flt, priority),               \
			    dmic_dfsdm_isr, DEVICE_DT_GET(flt), 0);            \
		irq_enable(DT_IRQN(flt));                                      \
	}                                                                      \
                                                                               \
	static const struct dmic_dfsdm_filter_cfg dmic_dfsdm_filter_##flt = {  \
		.irq_config_func = dmic_dfsdm_irq_cfg_func_##flt,              \
		.pcfg = PINCTRL_DT_DEV_CONFIG_GET(flt),                        \
		.parent = DEVICE_DT_GET(DT_PARENT(flt)),                       \
	};                                                                     \
                                                                               \
	static struct dmic_dfsdm_filter_data dmic_dfsdm_filter_data_##flt = {  \
		...
		.rx_queue = &dmic_dfsdm_msgq_##flt,                            \
	};                                                                     \
                                                                               \
	DEVICE_DT_DEFINE(flt, dmic_dfsdm_filter_init, NULL,                    \
			 &dmic_dfsdm_filter_data_##flt,                        \
			 &dmic_dfsdm_filter_##flt, POST_KERNEL,                \
			 CONFIG_AUDIO_DMIC_INIT_PRIORITY, &dmic_dfsdm_ops);

#define DFSDM_DEFINE(n)                                                        \
	PM_DEVICE_DT_INST_DEFINE(n, dmic_dfsdm_pm_action);                     \
                                                                               \
	static const struct dmic_dfsdm_cfg dmic_dfsdm_cfg_##n = {              \
		.pclken = STM32_DT_INST_CLOCK_INFO_BY_NAME(n, pclk),           \
		.aclken = STM32_DT_INST_CLOCK_INFO_BY_NAME(n, aclk),           \
		.reset = RESET_DT_SPEC_INST_GET(n),                            \
	};                                                                     \
                                                                               \
	DEVICE_DT_INST_DEFINE(n, dmic_dfsdm_init, NULL, NULL,                  \
			      &dmic_dfsdm_cfg_##n, POST_KERNEL,                \
