...
{
	const struct wm8994_driver_config *config = dev->config;
	struct wm8994_driver_data *data = dev->data;
	uint8_t buf[4];
	int ret;
...
static int wm8994_update_reg(const struct device *dev, uint16_t reg, uint16_t mask, uint16_t val)
{
	...
	return wm8994_write_reg(dev, reg, new_val);
}
...
static int wm8994_soft_reset(const struct device *dev)
{
	...
	ret = wm8994_write_reg(dev, WM8994_SOFTWARE_RESET, WM8994_SW_RESET_VALUE);
	if (ret < 0) {
		return ret;
	}
...
static int wm8994_driver_configure_clocking(const struct device *dev, struct audio_codec_cfg *cfg)
{
	const struct wm8994_driver_config *config = dev->config;
	...
	ret = wm8994_update_reg(
		dev, WM8994_AIF1_CLOCKING_1, WM8994_AIF1CLK_SRC_MASK | WM8994_AIF1CLK_ENA,
		(config->clock_source << WM8994_AIF1CLK_SRC_SHIFT) | WM8994_AIF1CLK_ENA);
	if (ret < 0) {
		return ret;
	}

	return wm8994_update_reg(dev, WM8994_AIF1_RATE,
				 WM8994_AIF1CLK_RATE_MASK | WM8994_AIF1_SR_MASK,
				 (aifclk_rate << WM8994_AIF1CLK_RATE_SHIFT) | sample_rate);
}

static int wm8994_driver_configure_audio_interface(const struct device *dev,
						   struct audio_codec_cfg *cfg)
{
	...
	ret = wm8994_update_reg(dev, WM8994_AIF1_CONTROL_1,
	...
	return wm8994_update_reg(dev, WM8994_AIF1_MASTER_SLAVE, WM8994_AIF1_MASTER, master);
}
...
static int wm8994_power_bias(const struct device *dev, bool enable)
{
	...
	ret = wm8994_update_reg(dev, WM8994_POWER_MANAGEMENT_1,
...
static int wm8994_route_playback(const struct device *dev, bool enable)
{
	...
	ret = wm8994_update_reg(dev, WM8994_POWER_MANAGEMENT_2,
				WM8994_PM2_MIXOUTL_ENA | WM8994_PM2_MIXOUTR_ENA, pm2);
	if (ret < 0) {
		return ret;
	}

	ret = wm8994_update_reg(dev, WM8994_POWER_MANAGEMENT_3,
				WM8994_PM3_DAC1L_ENA | WM8994_PM3_DAC1R_ENA, pm3);
	if (ret < 0) {
		return ret;
	}

	ret = wm8994_update_reg(dev, WM8994_POWER_MANAGEMENT_5,
				WM8994_PM5_AIF1DAC1L_ENA | WM8994_PM5_AIF1DAC1R_ENA, pm5);
	if (ret < 0) {
		return ret;
	}

	return wm8994_update_reg(dev, WM8994_POWER_MANAGEMENT_6,
...
static int wm8994_route_capture(const struct device *dev, bool enable)
{
	...
	ret = wm8994_update_reg(dev, WM8994_POWER_MANAGEMENT_4,
				WM8994_PM4_ADCL_ENA | WM8994_PM4_ADCR_ENA, pm4);
	if (ret < 0) {
		return ret;
	}

	return wm8994_update_reg(dev, WM8994_POWER_MANAGEMENT_5,
...
static int wm8994_set_dac_mute(const struct device *dev, bool mute)
{
	...
	ret = wm8994_update_reg(dev, WM8994_DAC1_FILTERS_1, WM8994_DAC1_MUTE_MASK, val);
	if (ret < 0) {
		return ret;
	}
					 ...
					 bool store)
{
	...
	case AUDIO_CHANNEL_HEADPHONE_LEFT:
		ret = wm8994_update_reg(dev, WM8994_DAC1_LEFT_VOLUME,
	...
	case AUDIO_CHANNEL_HEADPHONE_RIGHT:
		ret = wm8994_update_reg(dev, WM8994_DAC1_RIGHT_VOLUME,
					WM8994_DAC1_VOLUME_UPDATE | WM8994_DAC1_VOLUME_MASK,
					reg_val);
		if (ret < 0) {
			return ret;
					    ...
					    uint8_t volume, bool store)
{
	...
	case AUDIO_CHANNEL_FRONT_LEFT:
	case AUDIO_CHANNEL_HEADPHONE_LEFT:
		ret = wm8994_update_reg(dev, WM8994_LEFT_OUTPUT_VOLUME,
	...
	case AUDIO_CHANNEL_FRONT_RIGHT:
	case AUDIO_CHANNEL_HEADPHONE_RIGHT:
		ret = wm8994_update_reg(dev, WM8994_RIGHT_OUTPUT_VOLUME,
					WM8994_OUTPUT_VOLUME_UPDATE | WM8994_OUTPUT_VOLUME_MASK,
					reg_val);
		if (ret < 0) {
			return ret;
...
static int wm8994_driver_configure(const struct device *dev, struct audio_codec_cfg *cfg)
{
	struct wm8994_driver_data *data = dev->data;
	int ret;
	...
	ret = wm8994_driver_configure_clocking(dev, cfg);
	if (ret < 0) {
		return ret;
	}

	ret = wm8994_driver_configure_audio_interface(dev, cfg);
	if (ret < 0) {
		return ret;
	}
...
static DEVICE_API(audio_codec, wm8994_driver_api) = {
	.configure = wm8994_driver_configure,
...
static int wm8994_init(const struct device *dev)
{
	...
	ret = wm8994_read_reg(dev, WM8994_SOFTWARE_RESET, &device_id);
	if (ret < 0) {
		return ret;
	}
...
#define WM8994_INIT(inst)                                                                          \
	static struct wm8994_driver_data wm8994_driver_data_##inst;                                \
	static const struct wm8994_driver_config wm8994_driver_config_##inst = {                   \
		.i2c = I2C_DT_SPEC_INST_GET(inst),                                                 \
		.mclk_freq = DT_INST_PROP_OR(inst, mclk_frequency, 12288000U),                     \
		.clock_source = DT_INST_ENUM_IDX(inst, clock_source),                              \
	};                                                                                         \
	DEVICE_DT_INST_DEFINE(inst, wm8994_init, NULL, &wm8994_driver_data_##inst,                 \
			      &wm8994_driver_config_##inst, POST_KERNEL,                           \
